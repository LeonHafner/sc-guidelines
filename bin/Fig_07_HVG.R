...
parser <- arg_parser("Script for plotting Fig_07")

parser <- add_argument(parser, "--meta_string", help="Meta string argument", type="character")
parser <- add_argument(parser, "--path_string", help="Path string argument", type="character")
parser <- add_argument(parser, "--output", help="Output path", type="character")
...
for (scenario in c("atlas_hvg", "atlas-ub-conditions_hvg", "dataset_hvg", "dataset-ub-cells_hvg")) {
...
# Atlas_hvg
df.atlas <- df[scenario == "atlas_hvg"]
...
df.atlas_ub_conditions <- df[scenario == "atlas-ub-conditions_hvg"]
...
ggsave(plot = grid, filename = "Fig_07_HVG.png", width = 1803, height = 2048, units = "px")
