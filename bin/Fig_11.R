...
# Parse arguments
p <- arg_parser("Plot Fig_11")

p <- add_argument(p, "--path_string", help = "path to input h5ad")
p <- add_argument(p, "--output", help = "name of the output file")

argv <- parse_args(p)
path_string <- argv$path_string
output <- argv$output
...
ggsave(p, width = 3606, height = 4096, units = "px", dpi = 400, filename = output)
