...
p <- ggplot(prc, aes(x = recall, y = precision, color = method)) +
  ...
  annotate("text", x = 0.75, y = 0.21, label = paste(auc.scdd), size = 5, color = color.code[method == "scdd", hex]) +
  annotate("text", x = 0.75, y = 0.14, label = paste(auc.distinct), size = 5, color = color.code[method == "distinct", hex]) +
